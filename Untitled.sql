CREATE DATABASE  IF NOT EXISTS `elepig` /*!40100 DEFAULT CHARACTER SET utf8mb4 COLLATE utf8mb4_0900_ai_ci */;
USE `elepig`;
...
If JSON_EXTRACT(p_userJSON, "$.googleEmail") = p_googleEmail then
	...
	-- check if exists
	select count(googleEmail), userId into v_cnt, v_userId
      from user
    ...
    -- insert new user
					...
					VALUES
					(JSON_EXTRACT(p_userJSON, "$.email") ,
					JSON_EXTRACT(p_userJSON, "$.lastNamel") ,
                    ...
                    Case JSON_EXTRACT(p_userJSON, "$.GDPRInitialConsent") 
						When true then sysdate
						Else null
                    End,
					1,
                    1,
                    0,
                    JSON_EXTRACT(p_userJSON, "$.profileNamel") ,
					p_userJSON,
       ...
       CALL `elepig`.`createLog`('createUser', p_return_text, 'success', p_return_code, v_userId);
    ...
    -- return error
       ...
       CALL `elepig`.`createLog`('createUser', p_return_text, 'error', p_return_code, v_userId);
...
wallets:[	
	{"accountName":"myFortune","publicKey:"12322abef…..f56d654a"},
	{"accountName":"myOtherFortune","publicKey:"45365abef…..f56d654a"}
]}	*/
/*
SELECT `user`.`email`,
    `user`.`lastNamer`,
...
-- Dump completed on 2018-06-11 17:44:50
